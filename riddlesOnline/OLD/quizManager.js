...
/**
 * GameManager - מודול תקשורת WebSocket
        ...
        this.socket.onmessage = (event) => {
          try {
            ...
            switch (data.type) {
              ...
              case 'STATUS':
                // בדיקה אם זו הודעת סטטוס מיוחדת
                if (data.content && data.content.statusType) {
                  switch (data.content.statusType) {
                    case 'QUIZ':
                      if (this.listeners.onQuizReceived) {
                        this.listeners.onQuizReceived(data.content.data);
                      }
                      break;
                      
                    case 'ANSWER':
                      if (this.listeners.onAnswerReceived) {
                        this.listeners.onAnswerReceived(data.content.data, data.sender);
                      }
                      break;
                      
                    case 'END_QUIZ':
                      if (this.listeners.onQuizEnded) {
                        this.listeners.onQuizEnded(data.content.data);
                      }
                      break;
                      
                    default:
                      // טיפול בסטטוסים אחרים
                      if (this.listeners.onStatusUpdate) {
                        this.listeners.onStatusUpdate(data);
                      }
                  }
                } else if (this.listeners.onStatusUpdate) {
                  this.listeners.onStatusUpdate(data);
                }
                break;
